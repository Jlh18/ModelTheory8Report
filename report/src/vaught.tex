...
\linkto{upwards_lowenheim_skolem}{Upwards L\"{o}wenheim-Skolem} and its dependencies.
...
Meanwhile in subsection 7.2, for any $L$-language \code{T},
we design an $L$-structure \code{term\_model T},
...
and hence \code{term\_model} will be small enough.

We would hope that \code{term\_model T2} is a model of $T_{2}$,
which would give us a model of the right size.
However, for \code{term\_model} to be a model of $T$ we need that $T$
is \linkto{is_complete_theory}{maximal consistent}
and \linkto{henkization}{Henkin}, hence we need to extend $L_{2}$ and $T_{2}$ once more.
We do this in $7.3$.

In $7.4$ we compute the cardinality of \code{term\_model},
...
\begin{itemize}
  \item $T$ is a \linkto{is_complete_theory}{maximally consistent theory}
...
\subsection{Cardinality of \code{term\_model}}
...
We also prove this lemma in the \linkto{henkinization_cardinality}{next section}.
